...
impl PreAggregationProcessor<'_> {
    ...
    /// `requests__tenant_id`), so we match the lambda member to this branch's
    /// equivalent member and read through the branch member's alias. Mirrors
    /// the legacy planner's `columnsFor`: prefer an exact member match (a
    /// dimension from a joined cube is identical across branches), then fall
    /// back to the short member name within the branch's own cube (a cross-cube
    /// union member).
