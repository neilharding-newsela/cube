...
pub trait BaseTools {
    ...
    // Allocated params can be non-string scalars (e.g. a boolean/number
    // `securityContext` value allocated by the context-symbols proxy), so they
    // are deserialized as `FilterValue` and normalized to their canonical string
    // form at the `query_tools` boundary — the param channel stays `Vec<String>`.
